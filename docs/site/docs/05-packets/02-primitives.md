...
Типичный источник этой памяти - `IncomingPacket.Body`, окно в буфер
транспорта, которое живёт только до следующего чтения из того же читателя
(страница «Из сырого пакета»). `Read(Span<byte> output)` копирует байты
прямо в буфер вызывающего кода и ничего не выделяет сам.
