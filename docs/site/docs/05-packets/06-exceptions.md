...
`PacketFlow.Dispatch`/`TryDispatch` и `HandleAsync` ведут себя иначе:
пакет уже дошёл до посетителя или до `On<Имя>`, и вместо исключения
срабатывает событие `PacketFlow.OnTrailingBytes` - устройство события
разобрано в [«Обработчиках и неизвестных пакетах»](04-handlers.md).
Лишние байты почти всегда значат, что спецификация пакета для этой
версии протокола описана неверно - это находка для отчёта в спеки,
а не сбой соединения.
