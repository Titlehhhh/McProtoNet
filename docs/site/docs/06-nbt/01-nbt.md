...
Тело пакета, из которого читается NBT, - окно в буфер транспорта: оно
живёт только до следующего чтения из пайпа, разбирать его нужно
сразу, не через `await`, как и остальные поля пакета. `ReadNbtTag`
