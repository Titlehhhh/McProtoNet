...
`IncomingPacket.Body` - окно в буфер транспорта в обоих случаях, но
граница разная. У `MinecraftConnection` тело живёт до следующего
`ReadPacketAsync`. У `StreamingConnection` живёт вся пачка: тело любого
пакета портится с началом следующего `ReadBatchAsync`, даже если из
прошлой пачки разобрали не все пакеты. Данные, нужные дольше, копируют
явно, как описано в «Потоке пакетов».

Параллельные чтения запрещены на обоих типах: второй `ReadPacketAsync`
или `ReadBatchAsync` поверх незавершённого первого - это
...
можно менять только между кадрами - вызов посреди чтения или записи
тоже бросает исключение. `StreamingConnection` этого выбора не даёт
вообще: шифр и порог сжатия фиксируются в момент `ToStreaming()`.
