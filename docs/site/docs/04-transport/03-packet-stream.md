...
а переставляет `Phase` код приложения, как описано в
[«Фазе и направлении»](../05-packets/01-phases-and-direction.md).
...
бросает исключение. Чистый разрыв, когда сервер закрыл поток между
кадрами, - это `EndOfStreamException`; то же исключение прилетает и при
обрыве посередине кадра, и обычный `foreach` эти два случая не различит.

Разрыв со своей стороны - через `Abort` или `DisposeAsync` - выглядит
иначе. Чтение, которое в этот момент ждало байты, получает
`ConnectionAbortedException` с причиной закрытия внутри. Чтение, начатое
уже после того, как `DisposeAsync` завершился, получает
...
Чтение так не защищено: транспорт разрешает только одно чтение сразу,
а второй параллельный вызов `ReadPacketAsync` - это
`InvalidOperationException`, ошибка вызывающего кода, а не гонка данных.
У `ReadPacketsAsync` обхода для этого нет: пока `await foreach` не отдал
управление обратно, второе чтение той же связи начинать нельзя.

## Закрытие

`DisposeAsync` сначала прерывает соединение, потом ждёт гейт отправки
(не дольше пяти секунд) и только затем возвращает буферы в пул. После этого
любой вызов бросает `ObjectDisposedException`. Порядок и таблица исключений -
в [«Отмене, ошибках, закрытии»](06-cancellation.md).
