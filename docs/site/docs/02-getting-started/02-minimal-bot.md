---
description: 'Консольный бот на McProtoNet за 15 минут: подключение, вход на offline-сервер, keep-alive и чтение здоровья.'
keywords: [minecraft, bot, csharp, quickstart]
---
...
Библиотека не крутит эту машину состояний за вас (см. [Чего библиотека не делает](/docs/overview/non-goals)). Она даёт сгенерированный базовый класс обработчика с одним методом `On<ИмяПакета>` на каждый пакет и свойство `Phase`, которое вы переключаете, когда сервер говорит.
...
- [Чего библиотека не делает](/docs/overview/non-goals) — до того, как планировать поиск пути.
- [Словарь](/docs/reference/glossary) — кадр, фаза, ординал, каталог и другие слова из API.
- Справка API — в работе; пока смотрите XML-комментарии к `MinecraftClient` и `ClientboundHandler`.
