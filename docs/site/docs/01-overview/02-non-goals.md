---
description: 'Что McProtoNet намеренно не делает: игровая логика, интерфейс, интеграции и готовые рецепты остаются за вами.'
---
...
Эта библиотека берет на себя весь ужас работы с протоколом Minecraft: сжатие пакетов, шифрование, сериализация и десериализация конкретных пакетов, управление пакетами, мультиверсия, а также низкоуровневое чтение и запись данных. Однако есть вещи, которые **остаются за вами**:
...
- Управление фазами протокола(handsahking, login, configuration, play).
