...
Пакет в этом цикле взят взаймы: его тело - окно в блок из пула, а не собственная
копия, и на следующем шаге цикл этот блок отпускает. Разбирать надо сразу. Если
тело нужно дольше, у цикла берут собственную ссылку через `Retain` и этот пакет
освобождают сами ([«Кто владеет телом пакета»](../04-transport/03-packet-stream.md)).
