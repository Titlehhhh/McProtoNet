...
Пакет, для которого метод не переопределён, всё равно разбирается, а потом
тихо отбрасывается: сервер шлёт много такого, что боту не нужно. В `OnUnknown`
он попадает по другой причине - когда номер не разобрался в реестре для этой
...
Первая - keep-alive. Сервер шлёт число и ждёт его обратно; молчащего клиента
он считает зависшим и рвёт соединение. Ответ уже был выше, в разделе
«Обработчик».

Вторая - чат. Сообщение игрока приходит пакетом `PlayerChatPacket`; тело
сообщения лежит готовой строкой в `PlainMessage`, а имя отправителя и
...
`SystemChatPacket` в метод `OnSystemChat`, и текст в нём лежит компонентом
NBT. Библиотека отдаёт компонент как есть; собирать из него строку - работа
приложения.

Своё сообщение уходит пакетом `ChatMessagePacket`. Кроме текста он несёт
время, соль и подпись, а следом - подтверждение чужих сообщений: смещение
`Offset`, ровно три байта `Acknowledged` и контрольную сумму `Checksum`.
Сервер, который не проверяет подписи, принимает нули.
...
Третья - собственное положение. Его несёт `PositionPacket`: три координаты и
флаги движения - на земле ли клиент и упёрся ли он в стену. Пакет уходит не
разово, а пока бот двигается; без него для сервера он стоит там, куда его
поставил последний телепорт.
