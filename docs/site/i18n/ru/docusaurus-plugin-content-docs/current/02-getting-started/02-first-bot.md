...
Пакет в этом цикле взят взаймы. Его тело - окно в блок из пула, а не
собственная копия, и на следующем шаге цикл этот блок отпускает. Разбирать надо
сразу. Чтобы тело жило дольше, возьмите ссылку через `Retain` и освободите её
сами ([«Кто владеет телом пакета»](../04-transport/03-packet-stream.md)).
