...
`ClientboundHandler` их 156, плюс `OnUnknown` на всё остальное. По умолчанию
...
наследует `ClientboundHandler` и переопределяет 16 пакетных методов из 156 плюс
...
`raw` взят взаймы на время вызова: тело - окно в блок из пула, которым владеет
цикл чтения, и на следующем шаге цикл его отпускает
([«Кто владеет телом пакета»](../04-transport/03-packet-stream.md)) - если тело
нужно дальше, здесь же берут свою ссылку через `Retain` или копируют байты.
