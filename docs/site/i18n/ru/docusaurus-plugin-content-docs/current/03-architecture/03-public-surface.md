...
([«Фаза и направление»](../05-packets/01-phases-and-direction.md)). Номер,
которого реестр не знает для этой версии, фазы и направления, приходит
в `OnUnknown`.
...
пакету поверх обычного `Stream`, без `MinecraftConnection`.
