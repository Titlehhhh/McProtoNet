...
и [`MinecraftClient`](../08-api-reference/McProtoNet/MinecraftClient.md); у
...
ожидание. На уровне `MinecraftClient` порядок такой: прервать соединение, если
оно ещё живо, дождаться гейта отправки, закрыть соединение. Гейт - тот самый,
который `SendAsync` и `SendRawAsync` занимают на время вызова, бюджет ожидания -
пять секунд, и соединение закрывается в любом случае, даже если гейт так и не
освободился. Соединение, отданное через `ToStreaming`, клиенту больше не
принадлежит и здесь не закрывается.

## Почему отмена начатого чтения рвёт соединение

Начатое чтение уже сидит внутри читателя кадров, и снаружи не видно, что именно
оно успело взять: часть кадра - длина или кусок тела - может быть вынута из
потока и осесть в буфере. Отдать её обратно нельзя, а значит, нельзя и вернуть
соединение к границе кадра для следующего вызова: кадр разорван так же, как при
обрыве сети или ошибке кодирования.
...
`CloseReason` - `null`, пока соединение открыто. После закрытия там лежит
исключение: либо то, что передали в `Abort`, либо первый сбой, который поймал
собственный читатель или писатель соединения. Конец потока - один из таких
сбоев - чтение, дошедшее до конца, бросает `EndOfStreamException`, и это же
исключение оседает в `CloseReason`. Совсем без причины закрывается только
потоковое соединение, через `CompleteAsync` или пустую последнюю пачку, и об
этом последний раздел страницы.

`Completion` - задача, которая завершается в момент закрытия и никогда не
падает: код может ждать её без `try/catch`, чтобы узнать факт закрытия, а затем
...
- [Соединение без клиента](04-raw-connection.md) - `MinecraftConnection` и
  потоковый путь без `MinecraftClient` сверху
- [Исключения](../05-packets/06-exceptions.md) - что бросает пакетный слой
  поверх транспорта
