...
посреди сессии, каждый своим действием кода приложения, и оба относятся к
транспорту в целом - к обоим направлениям сразу.
...
[`MinecraftClient`](../08-api-reference/McProtoNet/MinecraftClient.md) (то же
свойство есть у
...
Порог решает судьбу каждого исходящего пакета отдельно. Пакет короче порога
уходит как есть, но в кадре появляется дополнительный VarInt перед телом - 0,
означающий «не сжат». Пакет не короче порога сжимается, и в этом же месте кадра
стоит VarInt с исходным (несжатым) размером. На входящей стороне ноль в этом
поле - сигнал развернуть пакет как есть, отличное от нуля значение - сколько
байт распаковать. В протоколе этот формат описан на странице
...
[Encryption](https://minecraft.wiki/w/Java_Edition_protocol/Encryption). Общий
...
если он есть, конверт сжатия. При записи писатель сперва достраивает кадр
(длина, при необходимости - сжатие), и только затем шифрует получившиеся байты
на месте. При чтении наоборот: сырые байты из сокета сначала
проходят через шифр, и лишь потом код разбирает в них длину кадра и, если нужно,
распаковывает тело.

## Что делает приложение

Сжатие и шифрование не переключаются сами - оба включает код приложения, по
одной строке на каждый:
