...
Окно жизни данных пачка взамен не расширяет. Перечислитель освобождает пакет, на
котором стоял, на каждом шаге, так что окно - один пакет, как и у
`MinecraftConnection`.
...
взаймы, а шаг к следующему пакету освобождает предыдущий. Тело, нужное дольше,
на обоих путях удерживают через `Retain`.
