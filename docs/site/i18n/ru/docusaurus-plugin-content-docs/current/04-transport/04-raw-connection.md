...
За `ReadBatchAsync` у `StreamingConnection` стоит буферизованный читатель: один
...
Окно жизни данных пачка взамен не расширяет: перечислитель на каждом шаге
освобождает пакет, на котором стоял, так что окно - один пакет, как и у
`MinecraftConnection`.

## Ограничения и особенности

`IncomingPacket.Body` - окно в блок из пула в обоих случаях
([«Кто владеет телом пакета»](03-packet-stream.md)), и граница одна и та же. У
`MinecraftConnection` `ReadPacketAsync` отдаёт пакет вызывающему, и тот его
освобождает. Внутри пачки владелец - перечислитель: `Current` отдаёт копию
взаймы, а шаг к следующему пакету освобождает предыдущий. На обоих путях тело,
нужное дольше, удерживают через `Retain`.
