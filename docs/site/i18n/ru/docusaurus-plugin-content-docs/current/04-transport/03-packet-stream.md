...
взаймы, настоящую ссылку цикл отпускает перед следующим шагом, и тело живо
внутри одного витка.
...
другого потока - у цикла берут собственную ссылку через `Retain`, и этот пакет
освобождают отдельно.
...
данные уходят в код, который про пул ничего не знает, - но `Retain` не выделяет
памяти, так что держать сам пакет дешевле.
