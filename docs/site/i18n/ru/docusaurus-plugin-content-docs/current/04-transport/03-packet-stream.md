...
Где кончается кадр и как читается длина - в [«Кадрах»](02-framing.md).

Поток пакетов не знает, в какой фазе клиент - handshaking, login, configuration
или play: кадры через него идут одинаково во всех фазах. Фазы ведёт код
приложения ([«Фаза и направление»](../05-packets/01-phases-and-direction.md)).

## Кто владеет телом пакета

Тело пакета - не собственная копия байт, а окно в блок, который читатель
арендует у пула. Пакет держит ссылку на этот блок, и блок уходит обратно в пул,
когда отпущена последняя ссылка. При сжатии блоков два - под сжатые байты и под
распакованные - но первый уходит сразу после распаковки, и правило для `Body`
не меняется.

`ReadPacketAsync` отдаёт ссылку вызывающему. Пакет принадлежит ему, он же его и
освобождает:
...
`ReadPacketsAsync` не отдаёт, а одалживает. Каждый пакет перечисления взят
взаймы, и настоящую ссылку цикл отпускает перед следующим шагом. Тело живо
внутри одного витка цикла.

Чтобы тело пережило виток, возьмите собственную ссылку через `Retain`.
Удержанный пакет освобождается отдельно:
...
`Body.ToArray()` по-прежнему копирует байты, и копия по-прежнему уместна, когда
данные уходят в код, который про пул ничего не знает. `Retain` не выделяет
памяти, поэтому держать сам пакет им дешевле.

Куда дальше уходит разобранный пакет - какой метод обработчика он вызовет и что
происходит с неизвестными id - описано в
...
Полная картина - в [«Отмене, ошибках, закрытии»](06-cancellation.md).

## Отправка

`SendAsync` и `SendRawAsync` проходят через общий гейт - `SemaphoreSlim(1, 1)`
внутри [`MinecraftClient`](../08-api-reference/McProtoNet/MinecraftClient.md).
...
([«Отмене, ошибках, закрытии»](06-cancellation.md)). У `ReadPacketsAsync` обхода
...
исключений - в [«Отмене, ошибках, закрытии»](06-cancellation.md).
...
- [Обработчики и неизвестные пакеты](../05-packets/04-handlers.md) - куда уходит
  прочитанный пакет
