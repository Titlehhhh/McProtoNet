...
Модуль задаёт два предела: глубина вложенности compound-ов и списков - 512, а
длина строки - 65535 байт, предел самого формата, потому что длина строки
хранится в двух байтах. Оба предела
...
Тело пакета, из которого читается NBT, - окно в блок из пула: разбирать его
нужно, пока пакет ещё держит на блок ссылку, и не через `await`, как и остальные
поля пакета ([«Кто владеет телом пакета»](../04-transport/03-packet-stream.md)).
`ReadNbtTag` читает ту же память и должно быть вызвано в том же синхронном
