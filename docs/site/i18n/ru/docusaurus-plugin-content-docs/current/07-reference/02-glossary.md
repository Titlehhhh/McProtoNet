...
- **окно в буфер** - тело пакета не копия байт, а участок блока, арендованного
  у пула. Пакет держит на блок ссылку, `Retain` берёт ещё одну, а в пул блок
  уходит, когда отпущена последняя. Подробнее -
  [«Поток пакетов»](../04-transport/03-packet-stream.md).
