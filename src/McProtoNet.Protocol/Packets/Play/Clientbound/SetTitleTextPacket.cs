...
public sealed partial class SetTitleTextPacket : IServerPacket
{
    ...
    internal void Serialize(ref MinecraftPrimitiveWriter writer, int protocolVersion)
    {
        switch (protocolVersion)
        {
            case >= 755 and <= 764:
            {
                var fields = VFirst_764 ?? throw new InvalidOperationException("SetTitleText VFirst_764 missing.");
                writer.WriteString(fields.Text);
                return;
            }
            case >= 765 and <= MinecraftVersion.LatestProtocol:
            {
                var fields = V765_Last ?? throw new InvalidOperationException("SetTitleText V765_Last missing.");
                writer.WriteAnonymousNbtTag(fields.Text, protocolVersion);
                return;
            }
            default:
                ThrowHelper.ThrowProtocolNotSupported(nameof(ServerPlayPacket.SetTitleText), protocolVersion, SupportedVersionsStatic);
                return;
        }
    }
            ...
            case >= 765 and <= MinecraftVersion.LatestProtocol:
            {
                var fields = new V765_LastFields
                {
                    Text = reader.ReadAnonymousNbtTag(protocolVersion)
                        ?? throw new InvalidOperationException("SetTitleText Text missing.")
                };
                V765_Last = fields;
                return;
            }
